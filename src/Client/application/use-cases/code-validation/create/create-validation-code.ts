import AbstractEmailProvider from "src/Client/application/services/emailProvider";
...
export class CreateValidationCodeUseCase extends AbstractCreateValidationCodeUseCase {
    ...
    async execute({ email, operationToValidateType }: ICreateValidationCodeParams) {
        const emailValidationFounded = await this.validationRepository.findByEmailAndOperation({
            email,
            operationToValidateType
        });
        
        if (emailValidationFounded) throw new Error("Email validation type already exists for that user");
