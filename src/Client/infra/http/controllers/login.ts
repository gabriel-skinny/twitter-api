import { Body, Controller, Delete, HttpStatus, Ip, Param, ParseUUIDPipe, Patch, Post } from "@nestjs/common";
...
export class LoginController {
    ...
    @Delete("user")
    async deleteUser(@Param("id", ParseUUIDPipe) userId: string): Promise<BaseControllerMethodInterface> {
    ...
    async forgotPassword(@Body() email: string): Promise<BaseControllerMethodInterface> {
    ...
    @Post()
    async login(@Body() email: string, password: string, deviceType: "desktop" | "mobile", @Ip() ip: string): Promise<BaseControllerMethodInterface<{ loginToken: string }>> {
        const { loginToken } = await this.loginUseCase.execute({
            email,
            password,
            ip,
            deviceType: deviceType == "desktop" ? SessionDeviceTypesEnum.DESKTOP : SessionDeviceTypesEnum.MOBILE
        });
    ...
    @Post("logout")
    async logout(@Param("id", ParseUUIDPipe) userId: string, @Ip() ip: string): Promise<BaseControllerMethodInterface> {
    ...
    async logoutAllOtherSessions(@Param("id", ParseUUIDPipe) userId: string, @Ip() ip: string): Promise<BaseControllerMethodInterface> {
    ...
    @Patch("update-user")
    async updateUser(@Param("id", ParseUUIDPipe) id: string, @Body() user: UpdateUserDto): Promise<BaseControllerMethodInterface> {
    ...
    async updatePassword(@Param("id", ParseUUIDPipe) userId: string, @Body() newPassword: string): Promise<BaseControllerMethodInterface> {
